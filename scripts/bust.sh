...
# ---------- 4. Stamp the service worker's cache key (project addition) ----------
# The SW keys its cache name off the same token so there is exactly one version number in the
# project. Added per the toolkit's own references/service-worker.md. NOTE: re-running the
# toolkit installer overwrites this file and drops this block — test/invariants.test.js asserts
# the SW token still matches index.html, so that regression fails the suite rather than
# silently pinning every installed user to an old build.
for SW in sw.js public/sw.js service-worker.js public/service-worker.js; do
  if [[ -f "$SW" ]]; then
    sed "${SED_INPLACE[@]}" -E "s/const CB_TOKEN = '[^']*'/const CB_TOKEN = '${TOKEN}'/" "$SW"
    rm -f "${SW}.cbbak"
    [[ -z "$QUIET" ]] && echo "  ✓ service worker cache key → ${TOKEN} in $SW"
  fi
done
